...
from cyy_naive_pytorch_lib.data_structure.cuda_process_task_queue import CUDAProcessTaskQueue
...
def __get_f(device, inputs, targets,idx, model_with_loss, param_shape_dict):
    def f(*args):
        nonlocal inputs, targets, idx,model_with_loss, param_shape_dict, device
        ...
        for i, arg in enumerate(args):
            ...
            loss = model_with_loss.loss_fun(
                cur_model_snapshot(inputs), targets,idx)
...
def processor_fun(task, args):
    ...
    indices = indices_dict.get(str(worker_device))
...
def get_hessian_vector_product_func(model_with_loss: ModelWithLoss, batch):
    ...
    for device in devices:
        ...
        indices_dict[str(device)] = copy.deepcopy(batch[2]).to(device)
